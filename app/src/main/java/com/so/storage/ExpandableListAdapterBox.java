...
public class ExpandableListAdapterBox extends BaseExpandableListAdapter
        implements OnTextClickListener, OnGroupClickListener {

    public Context mContext;
    public ExpandableListView mListView;
    public ArrayList<Box_MainDTO> category_main;
    public ArrayList<Box_SubDTO> category_sub;
    static OnTextClickListener listener;
    static OnGroupClickListener glistener;

    public ExpandableListAdapterBox(Context pContext, ExpandableListView pListView, ArrayList<Box_MainDTO> category_main
            , ArrayList<Box_SubDTO> category_sub) {
    ...
    public void addItem(Box_MainDTO groupData, Box_SubDTO item) {
        if (!category_main.contains(groupData)) {
            category_main.add(groupData);
        }
        int ind = category_main.indexOf(groupData);
        ArrayList<Box_SubDTO> lstItems = category_main.get(ind).getItems();
        lstItems.add(item);
// category_main.get(ind).setItems(lstItems);
    ...
    public Object getChild(int groupPosition, int childPosition) {
        ArrayList<Box_SubDTO> item = (category_main.get(groupPosition)).getItems();
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View view,
                             ViewGroup parent) {
        Box_SubDTO item = (Box_SubDTO) getChild(groupPosition, childPosition);
// if (view == null) {
...
view.setBackgroundColor(Color.parseColor("#F9F9F9"));
    ...
    public View getGroupView(int groupPosition, boolean isLastChild, View view, ViewGroup parent) {
        Box_MainDTO model = (Box_MainDTO) getGroup(groupPosition);
// if (view == null) {
        LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(mContext.LAYOUT_INFLATER_SERVICE);
